...
def get_prkey():
	private_key_file_path = input("Enter your private_key file absolute path : ")
...
def ecc_key_1_generation(rmac,vid): # Used in the encryption VC1
	...
	#x^3 + VID * x + rmac
	key1 = x**3 + vid * x + int(rmac.replace(':',''),16)
...
def ecc_key_i_generation(rmac,key): # Used in the encryption VC1
	...
	data = key + bytes.fromhex(rmac.replace(':',''))
	h = hashlib.sha256(data).digest()
	x_key = h[:16]
	#convert the key to an int
	x = bytes_to_long(x_key)
	...
	#x^3 + (Key_i-1) * x + rmac
	key = x**3 + bytes_to_long(key) * x + int(rmac.replace(':',''),16)
	key = str(key).encode()
